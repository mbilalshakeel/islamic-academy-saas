-- ═══════════════════════════════════════════════════════════
-- 023_seed_tenant_defaults_stage2_extension.sql
-- Extends seed_tenant_defaults() to also copy the Stage 2 tables
-- (tenant_settings, dhikr_items, calendar_events) from the template
-- tenant into every newly provisioned tenant. aladhan_prayer_time_cache
-- is deliberately NOT seeded — it's a live, per-tenant, 24h-TTL cache
-- with no meaningful "default" content; a brand-new tenant simply has
-- an empty cache until its first Sehri/Iftar request populates it.
-- ═══════════════════════════════════════════════════════════
...
begin
  ...
  -- ── tenant_settings (1:1) — Stage 2 ───────────────────
  insert into tenant_settings (tenant_id, city, country, latitude, longitude, calculation_method,
                                currency, gold_price_per_gram, silver_price_per_gram, zakat_nisab_override)
    select p_new_tenant_id, city, country, latitude, longitude, calculation_method,
           currency, gold_price_per_gram, silver_price_per_gram, zakat_nisab_override
    from tenant_settings where tenant_id = v_template_id;

  -- ── dhikr_items — Stage 2 ─────────────────────────────
  insert into dhikr_items (tenant_id, arabic_text, transliteration, translation, default_target_count, category, sort_order, is_active)
    select p_new_tenant_id, arabic_text, transliteration, translation, default_target_count, category, sort_order, is_active
    from dhikr_items where tenant_id = v_template_id;

  -- ── calendar_events — Stage 2 ─────────────────────────
  insert into calendar_events (tenant_id, hijri_month, hijri_day, title, description, is_recurring_yearly, sort_order)
    select p_new_tenant_id, hijri_month, hijri_day, title, description, is_recurring_yearly, sort_order
    from calendar_events where tenant_id = v_template_id;

  -- NOTE: aladhan_prayer_time_cache is intentionally NOT seeded — see
